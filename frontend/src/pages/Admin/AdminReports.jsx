  ...
  const fetchReportsData = async () => {
    try {
      const [salesResponse, topItemsResponse] = await Promise.all([
        axios.get(
          `https://qr-based-kitchen.vercel.app/api/admin/reports/sales?period=${selectedPeriod}`
        ),
        axios.get(
          "https://qr-based-kitchen.vercel.app/api/admin/reports/top-items"
        ),
      ]);
    ...
    try {
      ...
      if (topItems && topItems.length > 0) {
        topItems.forEach((item, index) => {
          csvContent += `${index + 1},${item.name},${item.totalQuantity},${
            item.totalRevenue
          }\n`;
        });
      } else {
      ...
      a.href = url;
      a.download = `campus-cravings-report-${selectedPeriod}-${
